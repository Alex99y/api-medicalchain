...
module.exports = {
    create: function(req,res,callback){
        ...
        if (
            ...
            callback(response); card.remove(req); return;                        
        }
        var data = {
            $class : "org.hyperledger.composer.system.AddParticipant",
            targetRegistry : "resource:org.hyperledger.composer.system.ParticipantRegistry#org.example.empty.Medico",
            resources : [{
                $class: "org.example.empty.Medico",
                S: "Medico",
                pacientes: [],
                perfil: "resource:org.example.empty.PerfilPublico#" + req.body.idRegistry,
    ...
    getAll: function(req,res,callback){
        ...
        if ( typeof output.registries["org.example.empty.Medico"].assets == 'undefined' ){
            output = "{}";
        }else{
            output = JSON.stringify(output.registries["org.example.empty.Medico"].assets);
        }
        response = '{"status":"success","message":"'+ output+'"}'
        card.remove(req);
        callback(response);        
    ...
    getById: function(req,res,callback){
        ...
        if ( typeof output.registries["org.example.empty.Medico"].assets == 'undefined' ){
            ...
            if (typeof output.registries["org.example.empty.Medico"].assets[req.body.id] == 'undefined') {
                output = "{}";
            }else{
                output = output = JSON.stringify(output.registries["org.example.empty.Medico"].assets[req.body.id]);
            }
        }
        response = '{"status":"success","message":"'+ output+'"}'
        card.remove(req);
