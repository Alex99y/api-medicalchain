...
describe('Transaction operation', function() {
    this.timeout(60000);
    describe('Give access to a Doctor', function() {
      it('Should give access to a Doctor', function() {
          return chai.request(url)
              .post('/transaction/giveAccess')
              .type('multipart/form-data')
              .attach('card', fs.readFileSync('./info/clinica/ClinicaAdmin@basic-sample-network.card'), 'test.card')
              .field("idRegistry","TEST")
              .field("idDoctor","TEST")
              .then( function(res){
                  expect(res.status).to.be.equal(200);
              }).catch(function(err){
  ...
  describe('Revoque access to a Doctor', function() {
    it('Should revoque access to a Doctor', function() {
        return chai.request(url)
            .post('/transaction/revoqueAccess')
            .type('multipart/form-data')
            .attach('card', fs.readFileSync('./info/clinica/ClinicaAdmin@basic-sample-network.card'), 'test.card')
            .field("idRegistry","TEST")
            .field("idDoctor","TEST")
...
describe('Registry operation', function() {
    this.timeout(60000);
    describe('Upload Registry', function() {
      it('Should upload registry', function() {
          return chai.request(url)
              .post('/uploadRegistry')
              .type('multipart/form-data')
              .attach('card', fs.readFileSync('./info/clinica/ClinicaAdmin@basic-sample-network.card'), 'test.card')
              .attach('file', fs.readFileSync('./test/files/Examenes.tar'), 'Examenes.tar')
              .field("idRegistry","TEST")
              .field("version",0)
              .then( function(res){
                  expect(res.status).to.be.equal(200);
    ...
    describe('Download Registry', function() {
        it('Should download registry', function() {
            return chai.request(url)
                .post('/downloadRegistry')
                .type('multipart/form-data')
                .attach('card', fs.readFileSync('./info/clinica/ClinicaAdmin@basic-sample-network.card'), 'test.card')
                .field("idRegistry","TEST")
                .then( function(res){
      ...
      describe('Ping the network', function() {
        it('Should ping the network', function() {
            return chai.request(url)
                .post('/ping')
                .type('multipart/form-data')
                .attach('card', fs.readFileSync('./info/clinica/ClinicaAdmin@basic-sample-network.card'), 'test.card')
